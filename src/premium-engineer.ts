...
If any answer depends on current external provider/library/platform behavior that is not provable from the repository, put a precise, minimal request in researchRequests.
...
Do not invent current external provider behavior. If a material external fact is still missing, put only the exact missing facts in researchRequests.
...
async function collectFullEvidence(
  ...
  let used = 0;
  const budget = Math.min(config.maxContextBytes, 32_000);

  for (const file of dedupe(files).slice(0, 10)) {
    try {
...
function evidenceRefs(capsule: ContextCapsule, searchText: string): string[] {
  ...
  const searched = searchText
    ...
    .slice(0, 16);
  return dedupe([...cited, ...searched]).slice(0, 20);
}
...
function escalation(
  ...
  return {
    ...
    reason,
    questions: [
      'Resolve only the listed external facts from authoritative sources; do not redo repository analysis.'
    ],
...
function generationMeta(
  ...
  return {
    // LocalEngineerResult predates the report stage. Use planning for aggregate token
    // accounting while worker inference telemetry still labels the call as report.
    stage,
    ...
    sections.push(
      `Findings:\n${report.findings
        .map((finding, index) => {
          ...
          return `${index + 1}. ${finding.claim}${evidence}`;
        })
        .join('\n')}`
    );
  }
...
async function executeReadOnlyResearch(
  ...
  reportProgress({
    ...
    detail: input.goal,
    reasoningSummary:
      'This job will investigate repository evidence and produce a report. It will not create editable tasks or mutate files.',
  ...
  const investigation = investigatorSchema.parse(
    JSON.parse(investigationGeneration.content) as unknown
  );
  ...
  const refs = evidenceRefs(focusedCapsule, searchEvidence.text);
  const modelCalls: LocalEngineerResult['modelCalls'] = [
    generationMeta('investigation', investigationGeneration)
  ];

  if (investigation.researchRequests.length > 0 && !input.claudeGuidance?.trim()) {
    reportProgress({
      phase: 'investigation',
      action: 'Repository investigation complete; authoritative external facts are required',
      detail: investigation.researchRequests.join(' | '),
      reasoningSummary:
        'The local agent stopped before planning. Claude only needs to research the exact external gaps and return a compact evidence capsule.'
    });
    return {
      result: {
        status: 'needs-claude',
        ...
        investigation: {
          ...
          researchRequests: investigation.researchRequests
        },
        repairRounds: 0,
        changedFiles: [],
        diff: '',
  ...
  const report = reportSchema.parse(JSON.parse(reportGeneration.content) as unknown);
  modelCalls.push(generationMeta('planning', reportGeneration));

  if (report.researchRequests.length > 0 && !input.claudeGuidance?.trim()) {
    return {
      result: {
        status: 'needs-claude',
        phase: 'investigation',
        ...
        investigation: {
          searchQueries: investigation.searchQueries,
          evidenceFiles: fullEvidence.files,
          researchRequests: report.researchRequests
        },
        repairRounds: 0,
        changedFiles: [],
        diff: '',
        validation: [],
        escalation: escalation(report.summary, report.researchRequests, refs),
        modelCalls
      },
      changes: []
    };
  ...
  reportProgress({
    ...
    reasoningSummary:
      'The local agent produced the final evidence-backed report without entering planning, implementation, validation or repair.',
...
/**
 * Canonical local-agent entry point. Read-only requests use a research/report state
 * machine; mutating engineering requests retain repo intelligence + full engineering.
 */
export async function executePremiumLocalEngineer(
  ...
  if (isReadOnlyEngineerRequest(input)) {
    return await executeReadOnlyResearch(model, config, input);
  }
