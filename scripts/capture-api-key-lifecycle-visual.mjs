...
credentials.addOrReplaceKeychainCredential({
  ...
  providerId: 'openai',
  label: 'Lifecycle Sibling',
  organizationId: 'personal',
  secret: siblingSecret
});
...
async function clickText(cdp, selector, text) {
  return await evaluate(cdp, `(() => {
    ...
    if (!target) throw new Error(${JSON.stringify(`${text} not found`)});
    target.click();
    return true;
  })()`);
}
...
try {
  ...
  await evaluate(cdp, `(() => {
    const card = document.querySelector('[data-connection-id="${firstConnectionId}"]');
    const button = [...(card?.querySelectorAll('button') ?? [])].find((item) => item.textContent?.trim() === 'Manage');
    if (!card || !button) throw new Error('Primary API Key Manage action not found');
    button.click();
    return true;
  })()`);
  ...
  const initial = await evaluate(cdp, `(() => {
    const dialog = document.querySelector('.api-key-manage-dialog');
    const password = dialog?.querySelector('input[type="password"]');
    const text = dialog?.textContent?.replace(/\\s+/g, ' ').trim() ?? '';
    return {
      hasCompany: text.includes('Company'),
      hasTest: text.includes('Test connection'),
      hasRotate: text.includes('Rotate API key'),
      hasRemove: text.includes('Remove connection'),
      passwordEmpty: password?.value === '',
      leaksOldSecret: text.includes(${JSON.stringify(firstSecret)}) || [...(dialog?.querySelectorAll('input') ?? [])].some((input) => input.value === ${JSON.stringify(firstSecret)})
    };
  })()`);
  if (!initial?.hasCompany || !initial.hasTest || !initial.hasRotate || !initial.hasRemove || !initial.passwordEmpty || initial.leaksOldSecret) {
    throw new Error(`Initial API lifecycle UI contract failed: ${JSON.stringify(initial)}`);
  }
  ...
  await evaluate(cdp, `(() => {
    const dialog = document.querySelector('.api-key-manage-dialog');
    const name = [...dialog.querySelectorAll('input')].find((input) => input.previousElementSibling?.textContent?.includes('Connection name'));
    const project = dialog.querySelector('input[aria-label="API header openai-project"]');
    if (!name || !project) throw new Error('Editable API lifecycle fields not found');
    const set = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set;
    set.call(name, 'Lifecycle Primary Edited');
    name.dispatchEvent(new Event('input', { bubbles: true }));
    set.call(project, 'project-smoke');
  ...
  await evaluate(cdp, `(() => {
    const input = [...document.querySelectorAll('.api-key-manage-dialog input[type="password"]')]
      .find((item) => item.previousElementSibling?.textContent?.includes('Replacement API key'));
    if (!input) throw new Error('Replacement API key input not found');
    const set = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set;
    set.call(input, ${JSON.stringify(rotatedSecret)});
  ...
  await evaluate(cdp, `(() => {
    ...
    if (!button) throw new Error('Disable API Key connection control not found');
    button.click();
    return true;
  })()`);
  ...
  const disabled = await evaluate(cdp, `(() => {
    const dialog = document.querySelector('.api-key-manage-dialog');
    const test = [...dialog.querySelectorAll('button')].find((button) => button.textContent?.includes('Test connection'));
    return { disabled: test?.disabled === true, text: dialog.textContent?.replace(/\\s+/g, ' ').trim() ?? '' };
  })()`);
  ...
  await evaluate(cdp, `(() => {
    const button = document.querySelector('.api-key-manage-dialog button[aria-label="Enable API Key connection"]');
    if (!button) throw new Error('Enable API Key connection control not found');
    button.click();
    return true;
  })()`);
  ...
  const remaining = await evaluate(cdp, `(() => ({
    firstGone: document.querySelector('[data-connection-id="${firstConnectionId}"]') === null,
    siblingPresent: document.querySelector('[data-connection-id="${siblingConnectionId}"]') !== null,
    text: document.querySelector('.connection-center-card')?.textContent?.replace(/\\s+/g, ' ').trim() ?? ''
  }))()`);
  if (!remaining?.firstGone || !remaining.siblingPresent || !remaining.text.includes('Lifecycle Sibling')) {
  ...
  try { credentials.remove(firstCredentialId); } catch { /* already removed by lifecycle or best-effort cleanup */ }
  try { credentials.remove(siblingCredentialId); } catch { /* best-effort cleanup */ }
