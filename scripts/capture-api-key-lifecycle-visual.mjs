...
try {
  ...
  const initial = await evaluate(cdp, `(() => { const dialog = document.querySelector('.api-key-manage-dialog'); const password = dialog?.querySelector('input[type="password"]'); const company = dialog?.querySelector('input[aria-label="API Key Company"]')?.value; const text = dialog?.textContent?.replace(/\\s+/g, ' ').trim() ?? ''; return { hasCompany: text.includes('Company'), company, hasTest: text.includes('Test connection'), hasRotate: text.includes('Rotate API key'), hasRemove: text.includes('Remove connection'), passwordEmpty: password?.value === '', leaksOldSecret: text.includes(${JSON.stringify(firstSecret)}) || [...(dialog?.querySelectorAll('input') ?? [])].some((input) => input.value === ${JSON.stringify(firstSecret)}) }; })()`);
  if (!initial?.hasCompany || initial.company !== 'Personal' || !initial.hasTest || !initial.hasRotate || !initial.hasRemove || !initial.passwordEmpty || initial.leaksOldSecret) throw new Error(`Initial API lifecycle UI contract failed: ${JSON.stringify(initial)}`);
